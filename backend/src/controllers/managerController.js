...
const { User, Worker, Assignment, Complaint } = require("../schemas");
...
const getAllWorkers = async (req, res) => {
  try {
    ...
    const formattedWorkers = await Promise.all(
      workers.map(async (worker) => {
        ...
        return {
          ...
          phone: worker.user_id ? worker.user_id.phone : worker.emergency_contact || "",
    ...
    console.error("Error fetching worker roster for manager:", error);
    return res.status(500).json({
      success: false,
